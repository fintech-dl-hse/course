...
- **2026-04-29 — RNNForward**: title `MathTex` two-line equation centered at `Y_TITLE=3.65` placed the BB top at `4.04`, just past the `+4.0` frame edge. Caught by `check_in_frame` (offscreen high). Fix: lower `Y_TITLE` by ~0.10 so the title BB top sits at ~3.94. Lesson: when computing the y-anchor for a multi-line `MathTex` title, budget at least half its rendered height (~0.4 for two lines at scale 0.55) below the `+4.0` ceiling, *plus* the `frame_margin=0.05` lint default. A safer default Y for two-line titles is ≤ 3.55. Already auto-detected by `check_in_frame`.
