...
1) Prefill — модель прогоняет ВЕСЬ входной префикс, считает и сохраняет KV‑Cache. Этот этап требует большое количество вычислений.

2) Decode — модель выдаёт по одному новому токену, переиспользуя KV‑Cache. На этом
этапе количество вычислений сильно меньше по сравнению с Prefill, но важна скорость задержка между сгенерированными токенами.
...
Написали запрос боту. Чтобы его обработать, бот берет системный промт и ваш запрос и запускает для него Prefill. Результат работы - KV-Cache сохраняется.
