...
md(
    """
#### ❓ **Вопрос**: чем `SimCLR` отличается от `CLIP`? Нужны ли негативы и есть ли риск коллапса в обоих?

<details>

<summary><strong>Ответ</strong></summary>

Оба — **contrastive** и оба **используют негативы**; коллапс (выдать константу на любой вход) грозит обоим, и спасают от него именно негативы. Главное различие — в **источнике позитивной пары**:</br>

* **SimCLR** — одна модальность. Естественной пары нет, поэтому позитив **изготавливают** двумя случайными аугментациями одной и той же картинки (опора на дизайн аугментаций). Негативы — другие картинки в батче.</br>
* **CLIP** — две модальности. Позитив даётся **бесплатно** из данных: картинка ↔ её настоящая подпись. Негативы — другие пары в батче (off-diagonal матрицы $N\\times N$).</br>

Важно: «бесплатным» в CLIP становится **позитив**, а не отказ от негативов. Без негативов схлопнулись бы оба — поэтому CLIP и обучают огромными батчами (~32k), чтобы негативов было много.</br>
По-настоящему **без негативов** работают non-contrastive методы (BYOL, DINO), но они платят асимметрией: predictor + stop-gradient + EMA-учитель / centering + sharpening. CLIP к ним не относится.
